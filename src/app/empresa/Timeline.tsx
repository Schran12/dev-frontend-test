...
export default function Timeline() {
  return (
    <section
      className="max-w-6xl mx-auto px-6 md:px-8 lg:px-12 py-16"
      aria-label="Linha do tempo da Bloomin"
    >
      <h2 className="text-3xl font-bold mb-12 text-center text-[#181c2c]">
        Nossa Trajetória
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {events.map((event, idx) => (
          ...
          >
            ...
            <h3 className="text-xl font-bold text-[#181c2c] mb-1 pl-8">
              {event.title}
            </h3>
            <time
              className="font-semibold text-gray-500 text-sm mb-2 block pl-8"
              dateTime={event.year}
            >
              {event.year}
            </time>
